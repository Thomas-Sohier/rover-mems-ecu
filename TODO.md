...
- [x] **`log.Fatal` inside the web server.** `web/server.go:113,123` call `log.Fatal` on bind/shutdown failure, killing the whole agent (incl. the active ECU connection) from a goroutine. Surface the error instead of exiting the process.
- [x] **Ignored serial read errors.** 8 sites use `n, _ := sp.Read(...)` (mems1x, mems19, mems3, rc5, readwrite). A hard error (unplugged USB) returns `n=0, err!=nil` and the code spins to timeout. Distinguish a hard error from an empty read.
- [x] **gofmt failures.** `ports_linux.go`, `ports_windows.go`, `loop_test.go` have mixed tabs/spaces. Run `gofmt -w .`.
...
- [x] **`interface{}` over `any`.** `State.LogDebug`/`LogDebugf` (`ecu.go:50,65`) use `interface{}`; repo targets go 1.23 and `any` is the convention (mems2j already uses it).
